...
/// الشاشة الرئيسية التي تضم شريط التنقل السفلي والصفحات الخمس
...
class _MainNavigationScreenState extends State<MainNavigationScreen> {
  ...
  Widget build(BuildContext context) {
    final List<Widget> screens = [
      HomeScreen(onTabChange: _onTabSelect), // 1. الشاشة الرئيسية الشاملة
      const ExploreToursScreen(),             // 2. شاشة استكشف الجولات المتطابقة تماماً مع فيجما
      const ExploreMapScreen(),               // 3. شاشة الخريطة التفاعلية
      const BookingsScreen(),                 // 4. شاشة الحجوزات والتذاكر الرقمية
      const ProfileScreen(),                  // 5. شاشة الملف الشخصي والإعدادات
    ];
...
/// شاشة استكشف الجولات - متطابقة 100% مع ترتيب فيجما الدقيق
...
class _ExploreToursScreenState extends State<ExploreToursScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      // الشريط العلوي المتطابق مع فيجما (السهم يسار، العنوان وسط، الفلتر يمين)
      body: SafeArea(
        child: Column(
          children: [
            Padding(
            ...
            // قائمة بطاقات الجولات المطابقة لفيجما (الصورة يسار، النصوص يمين)
            Expanded(
...
/// بطاقة الجولة السياحية المتطابقة تماماً مع تصميم فيجما (الصورة يسار، النصوص يمين)
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          child: Padding(
            ...
            // نستخدم LTR داخل الكرت ليظل ترتيب الصورة باليسار والنصوص باليمين تماماً مثل فيجما
            child: Directionality(
              textDirection: TextDirection.ltr,
              child: Row(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  // 1. صورة الجولة على اليسار مع الشارة وزر المفضلة
                  Stack(
                    children: [
                      ClipRRect(
                        borderRadius: BorderRadius.circular(16),
                        child: Image.network(
                          ...
                          errorBuilder: (context, error, stackTrace) => Container(
                            ...
                            child: const Center(
                              child: Icon(Icons.explore_rounded, color: Colors.white, size: 36),
                            ),
                          ),
                      ...
                      // شارة (الأكثر حجزاً) في الزاوية العلوية اليمنى للصورة
                      ...
                      // زر المفضلة في الزاوية العلوية اليسرى للصورة
                      Positioned(
                        top: 8,
                        left: 8,
                  ...
                  // 2. عمود بيانات الجولة على اليمين باتجاه عربي RTL
                  Expanded(
                    child: Directionality(
